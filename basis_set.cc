...
bool read_basis_set(const string &basis_set_path, WFN &wave, bool debug){
	...
	for(int i=0; i<wave.get_ncen ();i++){
		...
		for(int j=0; j<elements_list.size();j++){
			if(!elements_list[j].find(wave.get_atom_label(i))) found=true;
	...
	for (int i=0;i<elements_list.size();i++){
		...
		if(elements_list[i].find(" ")!=-1) elements_list[i].erase(elements_list[i].find(" "),1);
		...
		while ( line.find("}")==-1 && !ifile.eof() ){
			...
			for (int j=0;j<count;j++){
				...
				switch(c_temp){
					case 's':
					case 'S':
						temp[1]=temp[1]*pow((2*temp[0]/pi),0.75);
						break;
					case 'p':
					case 'P':
						temp[1]=temp[1]*pow((128*pow(temp[0],5)/pow(pi,3)),0.25);
						break;
					case 'd':
					case 'D':
						temp[1]=temp[1]*(pow(2048*pow(temp[0],7)/(9*pow(pi,3)),0.25));
						break;
					case 'f':
					case 'F':
						temp[1]=temp[1]*pow((32768*pow(temp[0],9)/(225*pow(pi,3))),0.25);
						break;
					default:
				...
				for(int h=0; h<wave.get_ncen(); h++){
					...
					if(temp_label.find(" ")!=-1) temp_label.erase(temp_label.find(" "),1);
					temp_label.append(":");
					if(elements_list[i].find(temp_label)!=-1){
						...
						switch(c_temp){
							case 's':
							case 'S':
								if(!wave.push_back_atom_basis_set (h,temp[0],temp[1],1,shell,debug_dens)){
							...
							case 'p':
							case 'P':
								if(!wave.push_back_atom_basis_set (h,temp[0],temp[1],2,shell,debug_dens)){
									cout << "ERROR while pushing back atoms basis set"<<endl;
									Enter();
								}
								if(debug_dens) cout << "Pushing back on atom: " << h+1 << " with coef: " << temp[1] 
							...
							case 'd':
							case 'D':
								if(!wave.push_back_atom_basis_set (h,temp[0],temp[1],3,shell,debug_dens)){
									cout << "ERROR while pushing back atoms basis set"<<endl;
									Enter();
								}
								if(debug_dens) cout << "Pushing back on atom: " << h+1 << " with coef: " << temp[1] 
							...
							case 'f':
							case 'F':
								if(!wave.push_back_atom_basis_set (h,temp[0],temp[1],4,shell,debug_dens)){
									cout << "ERROR while pushing back atoms basis set"<<endl;
									Enter();
								}
								if(debug_dens) cout << "Pushing back on atom: " << h+1 << " with coef: " << temp[1] 
...
bool read_basis_set_vanilla(const string &basis_set_path, WFN &wave, bool debug){
	if(debug) debug_dens=true;
	string basis_set_name;
	string temp;
	bool end=false;
